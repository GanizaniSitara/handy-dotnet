...
internal static class IconAssets
{
    ...
    public static BitmapSource RenderHand(int size, Color fillColor)
    {
        ...
        {
            const double srcW = 126, srcH = 135;
            var pad = size * 0.06;
            var avail = size - 2 * pad;
            var scale = Math.Min(avail / srcW, avail / srcH);
            var w = srcW * scale;
            var h = srcH * scale;
            dc.PushTransform(new TranslateTransform((size - w) / 2, (size - h) / 2));
            dc.PushTransform(new ScaleTransform(scale, scale));
            dc.DrawGeometry(brush, null, HandGeometry);
            dc.Pop();
            dc.Pop();
        }
